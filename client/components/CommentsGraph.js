...
class CommentsGraph extends React.Component {
	...
	renderChart() {
		...
		const y = d3.scaleLinear().domain([0, d3.max(this.props.media, d => d.comments.count)]).rangeRound([height, 0]);
		...
		topG.selectAll('bar').data(this.props.media)
			...
			.attr('y', d => y(d.comments.count))
			.attr('height', d => height - y(d.comments.count));
	}
